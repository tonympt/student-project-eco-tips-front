...
function AccordionTemplate({ items, options }) {
  ...
  return (
    <div ref={accordionRef} id="accordion-collapse" data-accordion="collapse">
      {items.map((item) => (
        <>
          <h2 id={item.id}>
            <button type="button" className="flex w-full justify-between text-lg p-2 shadow-md text-white font-extrabold bg-gradient-to-r to-green-400 from-emerald-600 opacity-80 border-b-4 border-green-500 rounded-t-lg" data-accordion-target={`#${item.bodyId}`} aria-expanded={item.active} aria-controls={item.bodyId}>
